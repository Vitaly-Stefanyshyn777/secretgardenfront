...
export const mapCoachToUi = (item: CoachApi): CoachUiItem => {
  ...
  // Використовуємо acf.city та acf.country як основні джерела, з fallback на location_city/location_country
  const city = 
    ensureString(item.acf?.city) || 
    joinToString(item.location_city) || 
    ensureString(item.acf?.location_city);
  const country = 
    ensureString(item.acf?.country) || 
    joinToString(item.location_country) || 
    ensureString(item.acf?.location_country);
