...
export default function ProductCharacteristicsPage({
  ...
  if (isLoading) {
    return (
      <ProductSubpageShell title="Характеристика та особливості" backHref={backHref}>
  ...
  if (isError || !product) {
    return (
      <ProductSubpageShell title="Характеристика та особливості" backHref={backHref}>
        <p className={styles.loading}>Не вдалося завантажити товар</p>
      </ProductSubpageShell>
    );
  }
  ...
  const endIdx = chars.findIndex((ch) => ch.name === "Важливі застереження");
        ...
        <div className={styles.charsBox}>
          {displayChars.map((ch, idx) => (
            <div key={idx} className={styles.charRow}>
              ...
              <span className={styles.charValue}>{ch.value}</span>
            </div>
          ))}
        </div>
      )}
