...
export const SizeFilter = ({ selectedSizes, onChange, loading }: SizeFilterProps) => {
  ...
  const terms = [...(termsData || [])]
    .reverse()
    .filter((term) => (term as { count?: number }).count !== 0) as Array<{
    id: number;
    name: string;
    slug: string;
    count?: number;
          ...
          <div className={styles.sizeButtons}>
            {[...Array(3)].map((_, i) => {
              const skeletonClasses = [
                styles.skeletonSizeButton90,
                styles.skeletonSizeButton100,
                styles.skeletonSizeButton85
              ];
              return (
                <Skeleton key={i} className={skeletonClasses[i]} />
              );
            })}
          </div>
        ) : isError ? (
            ...
            {terms.map((term) => (
              <button
                ...
                className={`${styles.sizeButton} ${
                  selectedSizes.includes(String(term.id)) ? styles.selected : ""
                }`}
                onClick={() => handleSizeToggle(String(term.id))}
              >
                {term.name}
              </button>
            ))}
